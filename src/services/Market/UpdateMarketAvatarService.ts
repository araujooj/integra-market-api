...
export default class UpdateMarketAvatarService {
    public async execute({ market_id, avatarFilename }: Request): Promise<Market> {
        ...
        if (market.avatar) {
            const marketAvatarFilePath = path.join(uploadConfig.directory, market.avatar);

            const marketAvatarFileExists = await fs.promises.stat(marketAvatarFilePath);
